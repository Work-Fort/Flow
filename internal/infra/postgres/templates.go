...
func (s *Store) CreateTemplate(ctx context.Context, t *domain.WorkflowTemplate) error {
	...
	_, err = tx.ExecContext(ctx,
		"INSERT INTO workflow_templates (id, name, description, version, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6)",
...
func (s *Store) GetTemplate(ctx context.Context, id string) (*domain.WorkflowTemplate, error) {
	...
	err := s.db.QueryRowContext(ctx,
		"SELECT id, name, description, version, created_at, updated_at FROM workflow_templates WHERE id = $1", id,
	...
	rows, err := s.db.QueryContext(ctx,
		...
		approval_mode, approval_required, approval_approver_role_id, approval_rejection_step_id
		FROM steps WHERE template_id = $1 ORDER BY position`, id)
	if err != nil {
	...
	rmrows, err := s.db.QueryContext(ctx,
		`SELECT id, template_id, step_id, role_id, allowed_actions FROM role_mappings WHERE template_id = $1`, id)
	if err != nil {
	...
	hrows, err := s.db.QueryContext(ctx,
		...
		FROM integration_hooks WHERE template_id = $1`, id)
	if err != nil {
...
func (s *Store) UpdateTemplate(ctx context.Context, t *domain.WorkflowTemplate) error {
	...
	res, err := tx.ExecContext(ctx,
		"UPDATE workflow_templates SET name = $1, description = $2, version = $3, updated_at = NOW() WHERE id = $4",
...
func (s *Store) DeleteTemplate(ctx context.Context, id string) error {
	...
	if err := s.db.QueryRowContext(ctx,
		"SELECT COUNT(*) FROM workflow_instances WHERE template_id = $1", id,
	).Scan(&count); err != nil {
	...
	res, err := s.db.ExecContext(ctx, "DELETE FROM workflow_templates WHERE id = $1", id)
	if err != nil {
